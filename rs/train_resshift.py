...
class ResShiftTraining:
    ...
    def build_dataloader(self):
        ...
        # make datasets
        datasets = {}
        datasets['train'] = RealESRGANDataset(
            self.configs.data["train"]["params"]
        )
        datasets['val'] = BaseData(
            **self.configs.data["val"]["params"]
        )
        ...
        # make dataloaders
        dataloaders = {'train': _wrap_loader(DataLoader(
                        datasets['train'],
                        batch_size=self.configs.train.batch[0],
                        shuffle=True,
                        drop_last=True,
    ...
    def train(self):
        self.resolve_resume_checkpoint()
        ...
        # gflops only
            ...
            print(f"FLOPS/px: {gflops/self.configs.data.train.params.gt_size**2}")
    ...
    def update_ema_model(self):
        ...
        rate = self.ema_rate
        for key, _ in self.ema_state.items():
...
def main():
        conf_path = "conf/swinunet_realesrgan256.yaml"

        print(torch.cuda.is_available())

        # zajisti konfiguraci
        configs = OmegaConf.load(conf_path)
        resshift = ResShiftTraining(configs)
        resshift.train()
