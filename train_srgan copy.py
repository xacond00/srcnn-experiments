...
base_model =  None #"working/4x96maeganh_rc3x16_d8x64b2B64.pth" #"gan_base/4x96_rc3x16.pth" if 0 else None
model_name = "auxresnet_maegan.pth" if srresnet else "4x96maeganh_rc3x16_d8x64b2B64.pth"
...
# Content loss
balance_cont = False # Balance content loss (increase weight with higher values)
cont_min = 0.01 # If balance loss it's minimum contribution else it's total content loss contribution
cont_mul = 1.0 # If balance loss is active, it multiplies content loss weight
...
def train_gan(train_loader, gen, disc, criterion, adv_criterion, optimizer_g, optimizer_d, epoch, grad_clip, print_freq, device, valid_ds = None):
    ...
    if(balance_loss):
        l = losses_d.avg()
        l = l if l < disc_lr_off else 1.0
        rate = max(min_disc_lr, min(l, max_disc_lr))
