...
def main():
    ...
    if(test):
        for i in range(50):
            compare_images(train_dataset, gen, device, i + 150, scaling_factor, test_downsample)
...
def train_gan(train_loader, gen, disc, criterion, optimizer_g, optimizer_d, epoch, grad_clip, device, valid_ds = None):
    ...
    for (lr_imgs, hr_imgs) in islice(train_loader, batches_per_epoch):
        ...
        if(dynamic_beta):
            t = min(loss_dis, 1)
            beta = beta * 0.5 + (t * beta_min + (1 - t) * beta_max) * 0.5
        scaler.update()
        # Track loss
    ...
    if(balance_loss):
        l = losses_d.avg()
        t = l if l < lr_disc_thr else 1.0
        rate = max(min_disc_lr, min(t * t, max_disc_lr))
        lr_disc_mul = disc_smooth * lr_disc_mul + (1 - disc_smooth) * rate
        lr_disc = lr * lr_disc_mul
        if(l < lr_disc_eps):
            lr_disc = 0
        for g in optimizer_d.param_groups:
            g['lr'] = lr_disc
