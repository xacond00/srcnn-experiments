...
base_model =  "gan_base/4x96_rc3x16th.pth" if 1 else None
model_name = "auxresnet_maegan.pth" if srresnet else "4x96mseganh_rc3x16th_d8x32.pth"
...
# Content loss
balance_cont = True # Balance content loss (increase weight with higher values)
cont_min = 0.01 # If balance loss it's minimum contribution else it's total content loss contribution
cont_mul = 5.0 # If balance loss is active, it multiplies content loss weight
...
disc_smooth = 0.8 # Smoothing factor of d_lr
disc_skip_thr = 0.0 # Dloss threshold for skipping update in batch
gen_skip_thr = 3.0 #1.0
cont_skip_thr = 0.5 #0.2 if loss_tp == 0 else 0.5
## Training params ##
...
iterations = 3600 * 10  # number of training iterations
...
grad_clip = None #0.1  # clip if gradients are exploding
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
cudnn.benchmark = True
checkpoint_ram = {}
checkpoint_safe = io.BytesIO()
...
def main():
    ...
    Training.
    """
    global start_epoch, epoch, checkpoint, min_loss, checkpoint_ram, lr, lr_disc, use_fp16
    ...
    if test:
        ...
        train_dataset = ImageDataset(dataset_name, ds_train, scaling_factor, pre_scale, crop_size, ds_cache, ds_precrop)
    if(test):
        for i in range(50):
    ...
    for epoch in range(start_epoch, epochs):
        #use_fp16 = local_epoch >= 10
        if(local_epoch % 100 == 0) and loss < 1:
            checkpoint_safe = io.BytesIO()
            print("Created backup checkpoint in epoch:", local_epoch)
            torch.save(checkpoint_ram, checkpoint_safe)
            checkpoint_safe.seek(0)
        ...
        loss_thr = closs_exit * (1 if loss_tp == 0 else 2.0)
        if(loss < loss_thr):
            checkpoint_ram = create_checkpoint(epoch,gen,disc,optimizer_d,optimizer_g, min_loss)
            min_loss = min(loss, min_loss)
        elif not unexplode_closs:
            fails += 1
            print(f"Loss has exploded {fails} times! Try tweaking the learning rate")
            checkpoint_ram = None
            with open(model_name, "wb") as f:
                f.write(checkpoint_safe.read())
            break
            #if fails >= 10: break
        local_epoch += 1
        #if(epoch):
...
def train_gan(train_loader, gen, disc, criterion, adv_criterion, optimizer_g, optimizer_d, epoch, grad_clip, print_freq, device, valid_ds = None):
    ...
    cont_weight = max(cont_adapt * cont_mul, cont_min) if balance_cont else cont_min
    skipped_disc = 0
    n_batches = min(len(train_loader), batches_per_epoch)
    for (lr_imgs, hr_imgs) in islice(train_loader, batches_per_epoch):
        ...
        with autocast_ctx: 
            ...
            skip_dis = unexplode_closs or (loss_con > cont_skip_thr and cont_skip_thr > 0)
            if(skip_dis):
                p_loss = c_loss
            else:
        ...
        #loss_con = c_loss.item()
        update_dis = not skip_dis and (loss_gen < gen_skip_thr or gen_skip_thr <= 0)
        if update_dis and not pause_dtraining:
            ...
            if loss_dis > disc_skip_thr or disc_skip_thr <= 0:
    ...
    if(balance_loss):
        ...
        l = losses_d.avg() + disc_lr_off
        rate = max(min_disc_lr, min(l * l, max_disc_lr))
        #rate = 1 - 2 * max(0.5 - l, 0)
        #rate = rate * rate
        lr_disc = disc_smooth * lr_disc + (1 - disc_smooth) * rate * lr
        #lr_disc = lr * max(min(ratio, 2.0), 0.002)
        for g in optimizer_d.param_groups:
            g['lr'] = lr_disc

    print(f'Epoch: [{epoch}]--'
        f'GPU ({gpu_time.sum:.3f}s)--'
        f'CPU ({cpu_time.sum:.3f}s)--'
        f'Epoch ({time.time() - tally:.3f}s)--'
        f'Closs ({losses_c.avg():.4f})--'
        f'Gloss ({losses_a.avg():.4f})--'
        f'Dloss ({losses_d.avg():.4f})--'
        f'Lr dis ({lr_disc:.2e})--'
        f'W closs ({cont_weight:.2e})--'
        f'Dskip ({skipped_disc / n_batches :.2f})')
    # Free memory
